...
- Actions:
  ...
  - openBoardFor(anchorId?: string, title?: string, dataset?: string): the board for a container — one call’s, or the space’s own — making it if nobody has yet. Returns its id either way. Call it from a click rather than on mount: creating a board writes records into a space everybody shares
