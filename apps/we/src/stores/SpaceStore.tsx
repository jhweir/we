...
export interface SpaceStore {
  ...
  posts: Accessor<(BlockType | undefined)[]>;
...
export function SpaceProvider(props: ParentProps) {
  ...
  const [posts, setPosts] = createSignal<(BlockType | undefined)[]>([]);
  ...
  // Recursive helper
  async function getBlockTree(parent: Block, perspective: PerspectiveProxy): Promise<BlockType | undefined> {
    try {
      ...
      const [block] = await (model as any).findAll(perspective, { source: parent.baseExpression });
